...
from .authorize import ready
from .authorize import webroot as webroot_validation
...
# Defaults
LETS_ENCRYPT_PRODUCTION = "https://acme-staging.api.letsencrypt.org/"
DEFAULT_ACCOUNT_PATH = 'account.json'
DEFAULT_AUTHZ_PATH = 'authz.json'
DEFAULT_WEBROOT_PATH = '~/'
...
def _authorize(args):
    ...
    answer = authorize(args.server, account, args.domain)
    # Write authz
    with open(args.authz_file, 'wb') as f:
        f.write(answer.encode('utf-8'))

def _ready(args):
    account = load_account(args.account)
    with open(args.authz_file, 'rb') as f:
        authz = f.read()
    ready(args.server, account, authz)

def _webroot(args):
    account = load_account(args.account)
    webroot_validation(args.server, account, args.domain, args.webroot)
...
def main():
    ...
    authorize = subparsers.add_parser(
        'authorize',
        help="Authorize for verify domain ownership",
        description=DESCRIPTION_AUTHORIZE,
        formatter_class=Formatter,
    )
    authorize.add_argument('domain', help="One or more domain names to authorize", nargs='+')
    authorize.add_argument('--authz-file', '-a', help="File for authorize information", default=DEFAULT_AUTHZ_PATH)
    ...
    verify.add_argument('--authz-file', '-a', help="File for authorize information", default=DEFAULT_AUTHZ_PATH)
    verify.set_defaults(func=_ready)

    # Domain webroot authorization
    webroot = subparsers.add_parser(
        'webroot',
        help="Verify domain ownership with webroot",
        description=DESCRIPTION_AUTHORIZE,
        formatter_class=Formatter,
    )
    webroot.add_argument('domain', help="One or more domain names to authorize", nargs='+')
    webroot.add_argument('--webroot', '-w', help="DOCUMENT_ROOT directory", default=DEFAULT_WEBROOT_PATH)
    webroot.set_defaults(func=_webroot)
